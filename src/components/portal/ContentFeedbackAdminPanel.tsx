  ...
  const handleChangeStatus = (statusName) => {
    ...
    getUser(feedbackData.creator).then((user) => {
      ...
      const generalEmailData: GeneralEmailTemplateProps = {
        ...
        p3Content: `Preview: ${feedbackData.description.slice(0, 100)}...`,
      ...
      >
        <div className='flex h-full flex-col justify-between gap-3 overflow-y-scroll px-4 pt-10 sm:px-8'>
              ...
              <div className=''>
                ...
                <fieldset className='mt-4'>
                  ...
                  <div className='space-y-4'>
                    {portalData.statuses.map((status, index) => (
                      <div key={index} className='flex items-center'>
                        <input
                          id={index}
                          name='status'
                          type='radio'
                          defaultChecked={status.name === feedbackData.status}
                          onClick={() => handleChangeStatus(status.name)}
                          disabled={
                            status.name === feedbackData.status || loading
                          }
          ...
          <div className='mb-5 flex flex-col gap-2'>
            <h2 className='text-xs font-medium text-gray-500'>Actions</h2>
