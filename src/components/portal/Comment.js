    ...
    <div key={comment.id} className='flex space-x-3'>
      <div className='flex-shrink-0'>
        <div className='flex h-9 w-9 items-center justify-center rounded-full border border-indigo-300 bg-indigo-100'>
          <h1 className='text-indigo-500 '>{comment.users?.name?.charAt(0)}</h1>
        </div>
      </div>
      <div className='flex min-w-0 flex-1 flex-col gap-1'>
        ...
        <div className=' '>
          <p className='text-sm font-light text-gray-500'>{comment.body}</p>
        </div>
        <div className='flex items-center justify-between '>
          <div className='flex items-center gap-1.5'>
            <button type='button' onClick={() => handleThumbsUpComment()}>
              <svg
        ...
        {openReplies ? (
          <div>
            ...
            >
              <span className='mr-2'>⏷</span>Hide {repliesData.length} replies
            </button>
            <Replies
          ...
          repliesData?.length > 0 && (
            <div>
              ...
              >
                <span className='mr-2'>⏵</span>Show {repliesData.length} replies
              </button>
            </div>
          )
        )}
