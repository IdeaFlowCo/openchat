...
export default function StreamingChat({}) {
  ...
  const [messages, setMessages] = useState<MessageType[]>([
    ...
    // },
    {
      message: `Hi ${getFirstName(
        auth.user.name
      )}, I'm Orion, an extremely concise AI. To speak to me, click the mic icon, then say "${
        START_KEYWORDS[0]
      }" to start the message, and "${END_KEYWORD}" to end your message. Make sure to speak clearly and say the keywords clearly and with pause. How are you? `,
      sender: 'AI',
    },
  ]);
  ...
  const sendMessage = async (message: string) => {
    ...
    setMessages((prevMessages) => [
      ...prevMessages,
      {
        message: ' ' + lastSubmittedMessageCount.current + ': ' + message,
        sender: 'human',
      },
    ]);
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const keyword of START_KEYWORDS) {
        ...
        const startIndex = transcript.text
          .toLocaleLowerCase()
        ...
        if (startIndex !== -1) {
          ...
          const endIndex = transcript.text
            .toLocaleLowerCase()
            .indexOf(END_KEYWORD.toLocaleLowerCase());
          if (endIndex !== -1 && endIndex > startIndex) {
            // @Ra: normally we'd add a conditiona i.e. && endIndex > startIndex | need to account for transcript wobble
            ...
            if (!sendingDetectedMessage.current) {
              //#question why needed? to prevent duplicates? how relates to loading?
              await stopRecording();
